...
void tarjan(int cur, int fa)
{
    ...
    for(int k = head[cur]; k; k = _next[k])
    {
        ...
        if(!dfn[to])
        {
            ...
            test[cur] = min(test[cur], test[to]);
            if(low[to] >= dfn[cur] && cur != fa) isAP[cur] = true;
            if(cur == fa) child++;
        }
        else low[cur] = min(low[cur], dfn[to]), test[cur] = min(test[cur], test[to]);
        if(dfn[to] && test[cur] != low[cur])
        {
            cout << cur << " right: " << low[cur] << " wrong: " << test[cur] << endl;
        }
    }
